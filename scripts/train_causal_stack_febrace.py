...
# 1. LOAD FILES
CSV_PATH = 'data/escolas_ideb_2019_completo.csv'
df = pd.read_csv(
    CSV_PATH, 
    dtype={'projects_information': str, 'febrace_before_2019': int}, 
    usecols=[
        'VL_NOTA_MEDIA_2019',
        'febrace_before_2019',
        'Latitude', 
        ...
        'Municipal',
        'IDHM Educação (2010)',
        'school_has_modalities_other_than_high_school',
        'school_size_number',
        'QT_SALAS_UTILIZADAS',
        'IN_AGUA_POTAVEL',
        ...
        'IN_BANDA_LARGA',
        'teachers_mean_age',
        'teachers_degree_mean',
        'mean_classes_per_teacher', 
        'mean_schools_per_teacher',
        'ratio_has_specialization',
        'ratio_has_masters_degree', 
        'ratio_has_phd'
    ]
)
df['IN_PATIO'] = (df['IN_PATIO_COBERTO'] + df['IN_PATIO_DESCOBERTO']).astype(bool).astype(int)
df['IN_SALA_LEITURA_TODAS'] = (df['IN_BIBLIOTECA_SALA_LEITURA'] + df['IN_SALA_LEITURA']).astype(bool).astype(int)
df.drop(
    columns=['IN_PATIO_COBERTO', 'IN_PATIO_DESCOBERTO', 'IN_BIBLIOTECA_SALA_LEITURA', 'IN_SALA_LEITURA'],
    inplace=True
)
df = df[
    (~df['Latitude'].isnull()) & (~df['Longitude'].isnull())
].query('VL_NOTA_MEDIA_2019 > 0').fillna(0).sample(frac=0.25).copy()
...
st.write(
    "Here's our first attempt at implementing a causal model for IHDP simulated data set"
)
...
st.header('Choose train test split ratio')
...
if roc_auc_treatment_prediction > max_treatment_prediction_auc:
    st.header('Warning: the positivity assumption might be violated!')
    st.write(
...
#     eps=0.5, 
#     min_samples=10,
# ).fit(train_set_leaves_2d_embedding)
...
# Train causal model with the train set
# causal_model = CausalExtraTreesRegressor(covariates=covariates, treatment=treatment, knn_params={'n_neighbors': 3})
causal_model = XLearnerRegressor(covariates=covariates, treatment=treatment, random_state=42)
...
st.header('Predicted ATE over test set')
st.write(
